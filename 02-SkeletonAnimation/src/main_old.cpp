...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//Cyborg
	cyborgModelAnimate.loadModel("../models/cyborg/cyborg_anim.fbx");
	...
	//AmongUs
	modelAmongUs.loadModel("../models/download/AmongUs/amongus_anim.fbx");
...
void applicationLoop() {
	...
	modelMatrixGuardian = glm::translate(modelMatrixGuardian, glm::vec3(10.0f, 0.0f, 10.0f));
	modelMatrixGuardian = glm::rotate(modelMatrixGuardian, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

	modelMatrixCowboy = glm::translate(modelMatrixCowboy, glm::vec3(-10.0f, 0.0f, 10.0f));
	...
	while (psi) {
		...
		cyborgModelAnimate.setAnimationIndex(2); //para establecer que animación ocupar
		cyborgModelAnimate.render(modelMatrixCyborg);

		glm::mat4 modelMatrixCowboyBody = glm::mat4(modelMatrixCowboy);
		modelMatrixCowboyBody = glm::scale(modelMatrixCowboyBody, glm::vec3(0.001f, 0.001f, 0.001f));
		cowboyModelAnimate.render(modelMatrixCowboyBody);

		glm::mat4 modelMatrixGuardianBody = glm::mat4(modelMatrixGuardian);
		modelMatrixGuardianBody = glm::scale(modelMatrixGuardianBody, glm::vec3(0.001f, 0.001f, 0.001f));
		guardianModelAnimate.render(modelMatrixGuardianBody);

		//Among Us
		glm::mat4 modelMatrixAmongUsBody = glm::mat4(modelMatrixAmongUs);
		modelMatrixAmongUsBody = glm::scale(modelMatrixAmongUs, glm::vec3(0.025f, 0.025f, 0.025f));
		modelAmongUs.setAnimationIndex(0);
		modelAmongUs.render(modelMatrixAmongUsBody);
