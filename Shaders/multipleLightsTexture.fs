...
uniform final porcentajeMezcla = 0.3;

vec3 calculateDirectionalLight(Light light, vec3 direction){
	vec3 textureColor1 = vec3(texture(texture1, our_uv)); //colocar el color, sin componente alpha
	vec3 textureColor2 = vec3(texture(texture2, our_uv));
	vec3 finalColor = mix(textureColor1, textureColor2, porcentajeMezcla); //utlimo parametro es factor de mezcla 0.3 = 70% de primer argumento, 30% del segundo
